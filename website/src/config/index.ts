export const NAV_IMG = {
  logo: {
    night: '/img/landingpage/night/Logo.png',
    light: '/img/landingpage/Logo.png',
  },
};
export const DEPLOY_IMG = {
  earth: {
    night: '/img/landingpage/night/World_night.svg',
    light: '/img/landingpage/World_day.svg',
  },
};

export const SwiperConfig = [
  {
    text: 'We use the Taichi programming language for our simulation projects. It is easy to use. 90% of the  team chose to use Taichi and completed their simulations with flying colors. It is also efficient. Taichi has much less scaffolding code than other programming languages.',
    link: 'https://taichi.graphics/',
    src: '/img/landingpage/ETH.svg',
    name: 'Jingwei Tang, Research Assistant',
  },
  {
    text: 'We use the Taichi programming language for our simulation projects. It is easy to use. 90% of the  team chose to use Taichi and completed their simulations with flying colors. It is also efficient. Taichi has much less scaffolding code than other programming languages.',
    link: 'https://taichi.graphics/',
    src: '/img/landingpage/ETH.svg',
    name: 'Alexander Mordvintsev',
  },
  {
    text: 'We use the Taichi programming language for our simulation projects. It is easy to use. 90% of the  team chose to use Taichi and completed their simulations with flying colors. It is also efficient. Taichi has much less scaffolding code than other programming languages.',
    link: 'https://taichi.graphics/',
    src: '/img/landingpage/ETH.svg',
    name: 'Alexander Mordvintsev',
  },
];
export const SWIPER_IMG = [
  {
    night: '/img/landingpage/night/User_Story_night.svg',
    light: '/img/landingpage/User_story_day.svg',
  },
  {
    night: '/img/landingpage/User_story_day.svg',
    light: '/img/landingpage/User_story_day.svg',
  },
  {
    night: '/img/landingpage/User_story_day.svg',
    light: '/img/landingpage/User_story_day.svg',
  },
];
export const communityImg = [
  {
    light: '/img/landingpage/Github footer.png',
    night: '/img/landingpage/night/Github footer.png',
    url: 'https://github.com/taichi-dev/taichi',
  },
  {
    light: '/img/landingpage/Slack footer.png',
    night: 'img/landingpage//night/Slack footer.png',
    url: 'https://taichicommunity.slack.com/join/shared_invite/zt-14ic8j6no-Fd~wKNpfskXLfqDr58Tddg#/shared-invite/email',
  },
  {
    light: '/img/landingpage/Forum footer.png',
    night: '/img/landingpage/night/Forum footer.png',
    url: 'https://github.com/taichi-dev/taichi/discussions',
  },
  {
    light: '/img/landingpage/论坛 footer.png',
    night: '/img/landingpage/night/论坛 footer.png',
    url: 'https://forum.taichi.graphics/',
  },
];

export const Run_rapidly_img = '/img/landingpage/Run-rapidly.svg';
export const Deploy_univerally_img = '/img/landingpage/Depoly_universally.svg';
