...
First of all, thank you for contributing! First of all, thank you for contributing! We welcome contributions of all forms, including but not limited to
...
- Please first leave a note (e.g. _I know how to fix this and would like to help!_) on the issue, so that people know someone is already working on it. This helps prevent redundant work; This helps prevent redundant work;
- If no core developer has commented and described a potential solution on the issue, please briefly describe your plan, and wait for a core developer to reply before you start. This helps keep implementations simple and effective. This helps keep implementations simple and effective.
...
- Almost every design decision has pros and cons. A decision is [good]{.title-ref} if its pros outweigh its cons. Always think about both sides. A decision is [good]{.title-ref} if its pros outweigh its cons. Always think about both sides.
- Debugging is hard. Debugging is hard. Changesets should be small so that sources of bugs can be easily pinpointed.
- Unit/integration tests are our friends.

::: note "There are two ways of constructing a software design: One way is to make it so simple that there are obviously no deficiencies, and the other way is to make it so complicated that there are no obvious deficiencies. _The first method is far more difficult_." — [C.A.R. Hoare](https://en.wikipedia.org/wiki/Tony_Hoare) ::: _The first method is far more difficult_." — [C.A.R. Hoare](https://en.wikipedia.org/wiki/Tony_Hoare) :::
...
Please base your discussion and feedback on facts, and not personal feelings. It is very important for all of us to maintain a friendly and blame-free community. Some examples: It is very important for all of us to maintain a friendly and blame-free community. Some examples:
...
- PRs with **small** changesets are preferred. PRs with **small** changesets are preferred. A PR should ideally address **only one issue**.
...
- Locally, you can run `ti format` in the command line to re-format code style. Locally, you can run `ti format` in the command line to re-format code style. Note that you have to install `clang-format-6.0` and `yapf v0.29.0` locally before you use `ti format`.

- If you don't have to install these formatting tools locally, use the **format server**. It\'s an online version of `ti format`. It\'s an online version of `ti format`.
  ...
  - Note that commit messages marked with `[format]` will automatically trigger the format server. e.g. `[format] your commit message` e.g. `[format] your commit message`
...
We do appreciate all kinds of contributions, yet we should not expose the title of every PR to end-users. We do appreciate all kinds of contributions, yet we should not expose the title of every PR to end-users. Therefore the changelog will distinguish [what the user should know]{.title-ref} from [what the developers are doing]{.title-ref}. This is done by **capitalizing PR tags**: This is done by **capitalizing PR tags**:
